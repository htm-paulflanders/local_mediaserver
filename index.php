 ...
 * @id         $Id: index.php 4439 2016-10-21 11:27:58Z pholden $
 */
...
$heading = get_string('pluginname', 'local_mediaserver');

$PAGE->set_context($context);
$PAGE->set_pagelayout('standard');
$PAGE->set_pagetype('local-mediaserver-index');
...
if (count($move) > 0) {
    $SESSION->local_mediaserver_move['stream'] = $move;
}
...
if ($moving && $confirm) {
    ...
    if (array_key_exists('stream', $SESSION->local_mediaserver_move)) {
        list($select, $params) = $DB->get_in_or_equal($SESSION->local_mediaserver_move['stream'], SQL_PARAMS_NAMED, 's');
    ...
    // Finished, remove the session variable and reload page.
