 ...
 * @id         $Id: lib.php 4826 2018-08-30 10:18:12Z pholden $
 */
...
function local_mediaserver_extend_navigation(global_navigation $navigation) {
    ...
    if (has_capability('local/mediaserver:view', $context)) {
        ...
        $container->showinflatnavigation = true;
        ...
        if (has_capability('local/mediaserver:viewepg', $context)) {
            $branch = $container->add(get_string('sourceepg', 'local_mediaserver'), new local_mediaserver_url('/local/mediaserver/guide/index.php'),
                navigation_node::TYPE_CONTAINER, null, 'epg');
 ...
 * @return array
 */
function local_mediaserver_user_preferences() {
    $preferences = array(
        'local_mediaserver_channels' => array('type' => PARAM_SEQUENCE, 'null' => NULL_NOT_ALLOWED, 'default' => 0),
    );
