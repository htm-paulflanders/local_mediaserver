 ...
 * @id         $Id: renderer.php 4682 2017-11-14 14:38:38Z pholden $
 */
...
class local_mediaserver_guide_renderer extends plugin_renderer_base {
    ...
    public function output_channel_programs(stdClass $channel, array $programs, $timebegin, $timeend) {
        ...
        foreach ($programs as $program) {
            ...
            $data = json_encode(array(
                'time' => local_mediaserver_local_time($program->timebegin, $strtimeformat),
                'duration' => format_time($program->timeend - $program->timebegin),
    ...
    public function output_program(stdClass $program, $time, $searchterms = '') {
        ...
        // Insert permalink element before program.
        $output  = html_writer::tag('a', '', array('id' => 'p' . $program->id)); 

        $output .= html_writer::start_tag('div', array('class' => implode(' ', $classes)));
        $output .= html_writer::tag('div', $title, array('class' => 'title'));
